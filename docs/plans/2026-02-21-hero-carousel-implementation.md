...
**Goal:** ホームのヒーローカルーセルをラグジュアリーブランド風にリデザインする。動画（Pexels API）+画像（Ken Burns）、1200msクロスフェード、プログレスバーナビゲーション。

**Architecture:** `HeroCarousel.tsx` を全面リファクタリング。Pexels APIで動画1本（1枚目スライド）、Unsplash APIで画像4枚（2〜5枚目）。Tailwindにアニメーション定義追加。

**Tech Stack:** React, TypeScript, Tailwind CSS, Pexels Videos API, Unsplash API

---

### Task 1: Tailwind設定にアニメーショントークンを追加

**Files:**
- Modify: `frontend/tailwind.config.ts`

**Step 1: tailwind.config.ts を編集して以下を追加する**

`transitionDuration` に `luxe: '1200ms'` を追加:
```ts
transitionDuration: {
  fast: '150ms',
  base: '400ms',
...
`transitionTimingFunction` に `luxe` を追加:
```ts
transitionTimingFunction: {
  elegant: 'cubic-bezier(0.25, 0.1, 0.25, 1.0)',
  luxe: 'cubic-bezier(0.4, 0, 0.2, 1)',   // ← 追加
},
```

`animation` に Ken Burns + stagger + progressFill を追加:
```ts
animation: {
...
git commit -m "feat: add luxe transition, Ken Burns, stagger, progress keyframes to tailwind"
```

---

### Task 2: .env.example に VITE_PEXELS_API_KEY を追加

**Files:**
- Modify: `frontend/.env.example`

**Step 1: .env.example 末尾に追記**

```
# Pexels (Hero Carousel video)
VITE_PEXELS_API_KEY=your_pexels_api_key_here
```

Pexels API キーの取得先: https://www.pexels.com/api/
（無料アカウントで取得可能）
...
git commit -m "docs: add VITE_PEXELS_API_KEY to env.example"
```

---

### Task 3: HeroCarousel.tsx を全面リファクタリング

**Files:**
- Modify: `frontend/src/components/home/HeroCarousel.tsx`

**Step 1: HeroCarousel.tsx をまるごと以下の内容に置き換える**
...
cd /mnt/c/Users/masat/AllApps/Connect40/frontend && npx tsc --noEmit
```
Expected: エラーなし（`any`/`unknown`型なし、TypeScript strict通過）
...
```bash
cd /mnt/c/Users/masat/AllApps/Connect40
git add frontend/src/components/home/HeroCarousel.tsx
git commit -m "feat: redesign HeroCarousel with video, Ken Burns, progress bar nav"
```
...
```bash
cd /mnt/c/Users/masat/AllApps/Connect40/frontend && npm run dev
```

**Step 2: ブラウザで確認するポイント**

- [ ] スライドが5枚（1枚目動画 or グラデーション、2〜5枚画像 or グラデーション）
- [ ] スライド切り替えが1200msのなめらかなクロスフェード
- [ ] 画像スライドで Ken Burns（ゆっくりパン・ズーム）が動いている
- [ ] テキストがスライド変わるたびにスタガードでフェードイン
- [ ] 画面下部にプログレスバーが表示され、時間経過でゴールドに塗られていく
- [ ] ホバーで矢印が薄く現れる
- [ ] ホバー中はプログレスバーが止まる
- [ ] `VITE_PEXELS_API_KEY` なしの場合、1枚目がグラデーションになる（エラーなし）

**Step 3: .env.local に Pexels キーを設定して動画も確認（任意）**

```bash
echo "VITE_PEXELS_API_KEY=your_actual_key" >> frontend/.env.local
```

その後、開発サーバーを再起動して1枚目スライドに動画が流れることを確認。

---

### Task 5: デプロイ

**Step 1: フロントエンドをビルド＆S3へデプロイ**
...
Expected: CloudFront invalidation が `InProgress` → 数分後に反映

**Step 2: 本番URLで確認**

https://d2s0k9gtsxu3ev.cloudfront.net を開いてホームページのカルーセルを確認。

---

## 注意事項

- `progressFill` keyframe はCSS `@keyframes` 名をそのまま `animation:` inline style で参照する。Tailwindの `animate-*` クラスではなくインラインstyleを使う（スライドごとに duration が異なるため）。
- Pexels API キーが未設定の場合、1枚目スライドは既存のグラデーションフォールバックになる。エラーや警告は一切出ない。
- Ken Burns アニメーションは `overflow-hidden` を持つ親コンテナ内でのみ正しくクリップされる。スライド `div` には `overflow-hidden` を必ず維持すること。
